...
Parent: [[Deployment/Deployment Overview|Deployment Overview]] · [[Security/Security Overview|Security Overview]]
...
Set `CONFIGFOUNDRY_AUTH_TRUSTED_PROXIES` to the proxy's IP/CIDR — see [[Security/Access Policy Engine#Trusted proxy resolution|Access Policy Engine § Trusted proxy resolution]] for why an unset/broad value is a real vulnerability.
...
SQLite is fine for a single-instance deployment of any size a small-to-mid team would produce. Move to PostgreSQL for concurrent write throughput beyond SQLite's single-writer model, or when policy requires a managed database server. See [[Database Overview]].
...
- Outbound: **none required at runtime** — see [[Deployment/Air-Gap Deployment|Air-Gap Deployment]].
...
**Identity & access:** `CONFIGFOUNDRY_AUTH_MFA_REQUIRED_ROLES` includes at minimum Super Admin and Organization Admin; custom roles reviewed against [[Security/RBAC Permission Catalog|RBAC Permission Catalog]] for least privilege; API keys scoped minimally with expirations set; a periodic access-review process exists.

**Data:** backup strategy in place and tested (see [[Operations/Runbook - Backup & Recovery|Runbook - Backup & Recovery]]); managed database infrastructure if not SQLite; audit log retention policy agreed with compliance.
...
The security layer is fully organization-scoped and ready for multiple tenants; the inventory tables are **not** — see [[Database Overview#Future schema improvements]]. If hard multi-tenant inventory isolation is required today, run separate instances per tenant rather than relying on organization boundaries to isolate inventory records.

## See also

[[Security/Security Overview|Security Overview]] · [[Security/SOC 2 Compliance Mapping|SOC 2 Compliance Mapping]] · [[Deployment/Deployment Overview|Deployment Overview]]
