...
const restoreSettings = (): Settings | null => {
  ...
  try {
    ...
    if (storedData) {
      ...
      // Defensive guard: localStorage is read back with no schema
      // validation. A stale/corrupted/hand-edited themeColor value (e.g.
      // 'customColors') gets used as a palette lookup key in
      // ThemeOptions.ts and crashes every MUI component that colors itself
      // via theme.palette.primary.main. Sanitize here so a bad persisted
      // value self-heals on next load instead of reproducing forever.
      if (!VALID_THEME_COLORS.includes(settings.themeColor)) {
