...
/**
 * Port of Vuexy's @core/layouts/components/shared-components/NotificationDropdown.tsx.
 * Only real behavior change from vendor: "Read All Notifications" is a real
 * link to /admin/audit-logs (the actual full feed this data comes from)
 * instead of just closing the menu -- `notifications` itself is populated by
 * the caller (AppBarContent.tsx) from real api.getAudit() rows, never
 * fabricated demo data like the vendor's hardcoded array.
 */
    ...
    <Fragment>
      ...
      >
        ...
        <ScrollWrapper hidden={hidden}>
          {notifications.length === 0 ? (
            <MenuItem disableRipple disableTouchRipple sx={{ cursor: 'default', userSelect: 'auto' }}>
              <Typography variant='body2' sx={{ color: 'text.disabled', width: '100%', textAlign: 'center' }}>
                No recent activity.
              </Typography>
            </MenuItem>
          ) : (
