...
Parent: [[Architecture/Decisions/ADR Index|ADR Index]] · [[API Documentation/API Versioning|API Versioning]]
...
**Negative:** the `StaticFiles` catch-all mount must always be registered after every `include_router()` call, an easy-to-violate ordering constraint with a silent failure mode (a new router registered after the mount is simply shadowed, not errored) — see [[API Documentation/API Versioning#StaticFiles ordering rule|API Versioning § StaticFiles ordering rule]]; a single shared app means all versions share one middleware stack — true version isolation (different auth/rate-limit behavior per version) would require the documented sub-application alternative instead.
...
[[API Documentation/API Versioning|API Versioning]] · [[Architecture Overview#API architecture]]
