...
flowchart TB
    subgraph API["api/v1/ (routers)"]
        ...
        A11[generate.py] --- A12[history.py] --- A13[export.py] --- A14[audit.py] --- A15[meta.py]
    end
...
Core dependency rule (see [[Architecture Overview#Principles]]): `api/` depends on `core/`; `core/` never depends on `api/`, `integrations/`, or `frontend/`. See [[Backend/Backend Overview|Backend Overview]] for what lives in each `core/` subpackage.
