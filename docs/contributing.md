...
Read [Architecture](./architecture.md) for the design principles this
...
[Security § Reporting a vulnerability](./security.md#reporting-a-vulnerability)
...
4. If you touched a dependency (Python or npm), regenerate the offline
   ...
   passes — see [Air-Gap Deployment](./airgap.md).
...
- Consistency with the layering rules in
  [Architecture](./architecture.md#layering-rules) — services don't
...
- **`static/networktree.js`** (the legacy pan/zoom Network Tree diagram)
  ...
  Next.js frontend (see [Roadmap](./roadmap.md)), test with a real
...
- **Any schema change is a new Alembic migration**, never an ad-hoc
  ...
  [Migrations § Writing a new migration](./migrations.md#writing-a-new-migration).

## Adding a new permission, provider, or integration

These are explicit extension points with existing patterns to follow:

- New permission code → [RBAC § Design rationale](./rbac.md#design-rationale)
- New storage provider → [Storage § Adding a new provider](./storage.md#adding-a-new-provider)
- New integration → keep it dependent on core, never the reverse, per
  [Architecture § Layering rules](./architecture.md#layering-rules)
