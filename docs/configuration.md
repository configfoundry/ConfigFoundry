...
|---|---|---|---|
| Provider | (in YAML `database.provider`) | `sqlite` | `sqlite`, `postgresql`, `mysql`, `sqlserver` — see [Storage](./storage.md) |
...
| `CONFIGFOUNDRY_AUTH_TRUSTED_PROXIES` | unset | Comma-separated CIDRs allowed to set `X-Forwarded-For` — see [Deployment](./deployment.md) |
...
See [Logging](./logging.md) for the full `LoggingConfig` reference
