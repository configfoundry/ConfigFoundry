...
and [Security](./security.md) (the full security model this checklist
draws from).
...
- [ ] `CONFIGFOUNDRY_AUTH_JWT_SECRET` set explicitly (not left to the
      random-at-startup default — see [Configuration](./configuration.md)).
...
- [ ] Custom roles reviewed against the [RBAC](./rbac.md) catalog —
...
- [ ] Database backup strategy in place and tested (see
      [Storage § Backups](./storage.md#backups)).
