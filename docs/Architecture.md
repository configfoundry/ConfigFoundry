...
6. Everything must work offline (see [Air-Gap Deployment](./airgap.md)).
...
For where these principles are headed next, see [Roadmap](./roadmap.md).
...
6. The route handler runs. Protected routes depend on
   ...
   the principal's role grants — see [RBAC](./rbac.md)).
...
- **[Authentication](./authentication.md)** / **[Authorization](./authorization.md)**
  / **[RBAC](./rbac.md)** — the full security layer: JWT + refresh
...
- **[API Versioning](./api-versioning.md)** — router-per-version design,
  how `/api/v2/` would be added without touching `/api/v1/`.
- **[Air-Gap Deployment](./airgap.md)** — why every dependency is
