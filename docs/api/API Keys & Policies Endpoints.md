...
- API key creation returns the full key value (`cfk_live_...`) once; only a hash and prefix are persisted (`APIKeyModel.key_hash`, `key_prefix`) — see [Database Overview](../architecture/Database Overview.md).
...
- Network ACL rules are evaluated first-match-wins by ascending `priority`; if no rule matches, the scope defaults to permissive **unless** at least one `allow` rule exists for that scope, in which case unmatched traffic is denied ("allowlist mode"). See [Access Policy Engine](../security/Access Policy Engine.md).
...
[Access Policy Engine](../security/Access Policy Engine.md) · [Feature - API Keys](../reference/features/Feature - API Keys.md) · [Feature - IP Access Policies](../reference/features/Feature - IP Access Policies.md)
