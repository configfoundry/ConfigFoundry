...
> [!IMPORTANT]
> Documented accurately against the current codebase: **no business-logic integrations (Datadog config push, NetBox sync, LDAP auth, SNMP discovery, ServiceNow, Slack, PowerDNS, Cloudflare, SNMP polling, etc.) are implemented today.** The one real, already-wired integration is **Datadog APM/observability instrumentation** (tracing, log correlation, profiling) — see [Datadog APM](Datadog APM.md) — which is a different thing from the business-integration contract described below. If your source of truth for this project (a requirements doc, a sales deck, etc.) lists SNMP, PowerDNS, Cloudflare, or SQL Server as "integrations," see the notes at the bottom of this page for where those actually live in this codebase.
...
3. May import core (`core.storage`, `core.domain`, `formats.yamldump`, etc.) — reads inventory, writes back only through the documented storage API.
...
## Planned integrations (from the contract's own examples + [Roadmap Overview](../roadmap/Roadmap Overview.md))
...
- **SQLite / PostgreSQL / MySQL / SQL Server** are **storage providers**, not integrations — they're part of core's own `StorageProvider` abstraction (`core/storage/`), not the optional `integrations/` layer. See [Database Overview](../architecture/Database Overview.md) and [Storage Abstraction](../architecture/Storage Abstraction.md).
...
- **Datadog** exists today, but only as observability instrumentation (traces/logs/profiling of ConfigFoundry itself), not as a business-logic "push config to Datadog" integration. See [Datadog APM](Datadog APM.md).

## See also

[Datadog APM](Datadog APM.md) · [Roadmap Overview](../roadmap/Roadmap Overview.md) · [Architecture Overview](../architecture/Architecture Overview.md#principles)
