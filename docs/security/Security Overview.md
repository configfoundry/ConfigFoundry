...
Single entry point for ConfigFoundry's security model — cross-links [Authentication](Authentication Overview.md), [Authorization & RBAC](Authorization & RBAC.md), [RBAC Permission Catalog](RBAC Permission Catalog.md), [Access Policy Engine](Access Policy Engine.md), and [SOC 2 Compliance Mapping](SOC 2 Compliance Mapping.md).
...
> [!NOTE]
> **Known limitation:** per-process — a multi-instance deployment enforces independent limits per instance, not one shared global limit. A Redis-backed limiter behind the same interface is a documented future step (v0.8.x, see [Roadmap Overview](../roadmap/Roadmap Overview.md)).

## Audit trail

Every security-relevant event (login, MFA change, role change, API key lifecycle, policy change) and every business-data mutation is recorded via `AuditRepository.log(...)`, queryable at `GET /api/v1/audit/search` (requires `audit:read`). Nothing is auto-purged — plan storage growth accordingly for compliance retention requirements. See [Database Overview](../architecture/Database Overview.md) for the `audit_log` schema.
...
|---|---|---|
| Secrets defaults | `CONFIGFOUNDRY_AUTH_JWT_SECRET` and `CONFIGFOUNDRY_AUTH_SECRET_ENC_KEY` default to a random value regenerated every restart in dev | **Must** be set explicitly before production go-live — see [Secrets & Configuration](Secrets & Configuration.md) and [Production Deployment](../deployment/Production Deployment.md) pre-go-live checklist |
...
| Legacy `api/*.py` routes | Unversioned route files sibling to `api/v1/` predate the auth layer in spirit; confirm they are genuinely unmounted (not imported by `app.py`) rather than a live, unauthenticated backdoor | Verify and delete/archive — see [Technical Debt](../development/Technical Debt.md) |
...
| GDPR-specific controls | Not built (data export/erasure, consent tracking) | Not committed to a timeline; audit trail and RBAC foundation support adding later — see [SOC 2 Compliance Mapping](SOC 2 Compliance Mapping.md) |
...
[Authentication](Authentication Overview.md) · [Authorization & RBAC](Authorization & RBAC.md) · [RBAC Permission Catalog](RBAC Permission Catalog.md) · [Access Policy Engine](Access Policy Engine.md) · [Secrets & Configuration](Secrets & Configuration.md) · [SOC 2 Compliance Mapping](SOC 2 Compliance Mapping.md)
