...
| Database | none required | SQLite ships built into Python; PostgreSQL/MySQL/SQL Server are optional (see [Storage](../architecture/storage.md)) |
...
[Release Process](../development/release-process.md)). It's a single self-contained
...
access" is verified: [Air-Gap Deployment](../deployment/airgap.md).
...
`vendor/npm/`, see [Air-Gap Deployment](../deployment/airgap.md#regenerating-the-offline-bundles))
...
[Air-Gap Deployment](../deployment/airgap.md) for what it does when `frontend/out/`
...
mid-sized teams. For larger deployments see [Storage](../architecture/storage.md) for
...
- [Air-Gap Deployment](../deployment/airgap.md) — the full offline-install story
- [Enterprise Deployment](../deployment/enterprise.md) — hardening checklist for production
