...
[Air-Gap Deployment](../deployment/airgap.md). Only regenerating the offline vendor
...
deployments. See [Storage](../architecture/storage.md).
...
scoping. See [Enterprise § Multi-tenancy](../deployment/enterprise.md#multi-tenancy).
...
than a shared global one. See [Deployment](../deployment/deployment.md).
...
for CDN/external-asset references — see [Air-Gap Deployment](../deployment/airgap.md).

**How often are dependencies updated?**
No fixed cadence currently — see [Release Process](../development/release-process.md)
for how a release is cut and [Roadmap](../roadmap/roadmap.md) for what's planned.

**I found a security issue. Where do I report it?**
See [Security § Reporting a vulnerability](../security/security.md#reporting-a-vulnerability).

**Can I add a new database provider / new integration?**
Yes — both are designed as extension points. See
[Storage § Adding a new provider](../architecture/storage.md#adding-a-new-provider) and
[Architecture § Layering rules](../architecture/architecture.md#layering-rules) for
