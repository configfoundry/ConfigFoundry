...
[Enterprise Deployment](../deployment/enterprise.md) and [Security](../security/security.md)
...
- **Generate YAML** — one config file per Collector Region, built from
  ...
  generation](../api/api.md#config-generation--history-requires-deploymentexecute--profileread)
...
> exclusively — see [Architecture § Frontend architecture](../architecture/architecture.md#frontend-architecture)
...
> [Roadmap](../roadmap/roadmap.md) as a real, open gap, not silently dropped.
...
- **Audit log + YAML history** — every change is attributed to whoever
  ...
  was produced and when. See [API Reference](../api/api.md) and
  [RBAC](../security/rbac.md#permission-catalog) for `audit:read`.
- **Minimal, well-known dependencies.** The backend runs on FastAPI,
  SQLAlchemy 2.x, Pydantic v2, and uvicorn — all vendored for fully
  offline installs (see [Air-Gap Deployment](../deployment/airgap.md)). AES-256-GCM
...
- **Safe upgrades.** Every schema change ships as a versioned, idempotent
  ...
  [Upgrade Guide](../deployment/upgrade.md) and [Migrations](../architecture/migrations.md).
...
- **Single-tenant inventory, multi-tenant auth.** The RBAC/API-key/policy
  ...
  [Enterprise § Multi-tenancy](../deployment/enterprise.md#multi-tenancy).
...
> concurrent write load. Move to PostgreSQL (see [Storage](../architecture/storage.md))
