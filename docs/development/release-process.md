...
[API Versioning](../api/api-versioning.md)), so API consumers are insulated
...
[Roadmap](../roadmap/roadmap.md) for what's likely to drive the next one.
...
only — see [Air-Gap Deployment § Regenerating the offline bundles](../deployment/airgap.md#regenerating-the-offline-bundles)).
...
- [Upgrade Guide](../deployment/upgrade.md) — applying a release to an existing install.
- [Air-Gap Deployment](../deployment/airgap.md) — the offline guarantees a release bundle upholds.
- [Installation](../getting-started/installation.md) — installing from a release bundle for the first time.
