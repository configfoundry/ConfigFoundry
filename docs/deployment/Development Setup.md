...
Parent: [Deployment Overview](Deployment Overview.md) · [Engineering Wiki](../development/Engineering Wiki.md)
...
A bare clone has no `frontend/out/` (generated, not committed) — running `python3 server.py` directly still works, falling back to the legacy `static/` UI. See [Frontend Overview](../architecture/Frontend Overview.md).
...
See [Database Overview](../architecture/Database Overview.md#migration-strategy) — edit the model, `alembic revision --autogenerate`, review the generated file, test upgrade and downgrade.
...
4. Add tests.
5. Note the change for the next release's [Changelog](../../CHANGELOG.md).
...
Explicit over clever (see [Architecture Overview](../architecture/Architecture Overview.md#principles)). No hardcoded role-name checks — permission codes only. No new external CDN/network dependency in application source (`validate_airgap.py` catches it). No Python linter/formatter is wired in — deliberate: one fewer pinned dependency to keep air-gap-clean, style enforced in review instead. See [Engineering Wiki](../development/Engineering Wiki.md) for the full standards.

## See also

[Engineering Wiki](../development/Engineering Wiki.md) · [Testing Strategy](../development/Testing Strategy.md) · [Deployment Overview](Deployment Overview.md)
