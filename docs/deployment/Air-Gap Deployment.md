...
Parent: [Deployment Overview](Deployment Overview.md) · [Architecture Overview](../architecture/Architecture Overview.md#principles)
...
Backs up the existing database (timestamped), copies it forward, re-runs the offline install against the **new** release's vendored dependencies (fresh `.venv/`, avoiding dependency drift). See [Upgrade & Rollback](Upgrade & Rollback.md).
...
[Deployment Overview](Deployment Overview.md) · [Upgrade & Rollback](Upgrade & Rollback.md) · [Datadog APM](../integrations/Datadog APM.md) (agent-reachability caveat under air-gap)
