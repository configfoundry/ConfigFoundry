...
Single process (FastAPI + static frontend export, same origin) backed by one database. No built-in load balancer, message queue, or worker pool — intended as one instance per team/environment, behind a reverse proxy for TLS termination. See [Deployment Diagram](../architecture/Deployment Diagram.md).
...
|---|---|---|
| Release bundle (`ConfigFoundry-Offline-vX.Y.Z.zip`) | Offline/air-gapped target | [Air-Gap Deployment](Air-Gap Deployment.md) |
| From source, with internet access | Local development, contributing | [Development Setup](Development Setup.md) |
| Offline install from a source checkout | Python-only offline install using the committed `vendor/python/` wheelhouse | [Air-Gap Deployment](Air-Gap Deployment.md) |

## Environment variables

Full reference: [Secrets & Configuration](../security/Secrets & Configuration.md).
...
No deployment automation beyond bundle publishing — actually rolling out a new version to a running instance is a manual step (see [Upgrade & Rollback](Upgrade & Rollback.md)).
...
See [Engineering Wiki § Versioning](../development/Engineering Wiki.md#versioning) and [Roadmap Overview](../roadmap/Roadmap Overview.md). Short version: semantic versioning, `frontend/package.json` is the source of truth, a release is cut when a coherent set of changes lands on `main` and passes CI (no fixed cadence), publishing is a single `git tag vX.Y.Z && git push origin vX.Y.Z`.

## Rollback

No `alembic downgrade` is recommended as the primary rollback path — restoring the pre-upgrade database file backup is simpler and doesn't depend on every migration having a correct, tested `downgrade()`. See [Upgrade & Rollback](Upgrade & Rollback.md).

## Scaling

Vertical scaling (bigger single instance) is the supported path today. Horizontal scaling requires PostgreSQL (not validated for SQLite) and accepts per-instance rate limiting until a Redis-backed limiter exists (v0.8.x). See [Roadmap Overview](../roadmap/Roadmap Overview.md) and [Security Overview § Rate limiting](../security/Security Overview.md#rate-limiting).

## Monitoring

See [Runbook - Monitoring & Health Checks](runbooks/Runbook - Monitoring & Health Checks.md).

## See also

[Development Setup](Development Setup.md) · [Production Deployment](Production Deployment.md) · [Air-Gap Deployment](Air-Gap Deployment.md) · [Upgrade & Rollback](Upgrade & Rollback.md)
