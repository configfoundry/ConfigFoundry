...
Parent: [[Repository Overview]] · [[Architecture Overview]] · [[Backend/Storage Abstraction|Storage Abstraction]]
...
SQLite is default per the "zero required backend dependencies" architecture principle. See [[Backend/Storage Abstraction|Storage Abstraction]] for the `StorageProvider` mechanics.
...
Rules (see [[Development/Engineering Wiki|Engineering Wiki]] for the full developer-facing version): never edit a shipped migration file; always use `op.batch_alter_table()` for changes to existing tables (SQLite doesn't support most `ALTER TABLE` forms directly); use SQLAlchemy column types, not dialect-specific SQL, unless gated on `bind.dialect.name`; commit the migration file alongside the model change in the same PR; test both `upgrade()` and `downgrade()`.

## Future schema improvements

- **Multi-tenant inventory scoping** — retrofit `org_id` onto `devices`/`bandwidth_caps`/`subnets`/`tag_defs`/`lists`/`yaml_history`, which are currently implicitly single-tenant even though the security layer around them is fully organization-scoped. Flagged as the largest single migration on the roadmap (v0.7.0) — see [[Roadmap Overview]] and [[Security/Security Overview#Known scope boundaries|Security Overview § Known scope boundaries]].
...
See the Mermaid diagram above, or [[Architecture/Diagrams/Data Flow|Data Flow]] for how data moves through these tables at request time.
...
See [[Operations/Runbook - Backup & Recovery|Runbook - Backup & Recovery]].

## See also

[[Backend/Storage Abstraction|Storage Abstraction]] · [[Security/Security Overview|Security Overview]] · [[Development/Engineering Wiki|Engineering Wiki]] · [[Operations/Runbook - Backup & Recovery|Runbook - Backup & Recovery]]
